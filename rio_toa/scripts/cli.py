...
@click.option('--l8-bidx', default=0,
    help="L8 Band that the src_path represents (Default is parsed from file name)")
@click.option('--stack', '-s', is_flag=True, default=False)
@click.option('--verbose', '-v', is_flag=True, default=False)
@click.option('--pixel-sunangle', '-p', is_flag=True, default=False, help="per pixel sun elevation")
@click.pass_context
@creation_options
def reflectance(ctx, src_paths, src_mtl, dst_path, dst_dtype, rescale_factor, readtemplate,
         workers, l8_bidx, stack, verbose, creation_options, pixel_sunangle):
